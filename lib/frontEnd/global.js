    ...
    let key_log_html = `
        <div class="col-3 key-log-card clickable" data-id="${key_log._id}">
            <div class="row">
                ...
                <div class="col-12 text-center pad-4">
                    <button class="btn btn-roku btn-block open-key-log" data-id="${key_log._id}">
                        Open
                    </button>
                </div>
            </div>
...
$('document').ready(function(e) {
    ...
    $('.sidebar-nav').click(function(e) {
        ...
        if(  nav_target === 'projects' ) {
            $('#tools-pane, #refs-pane, #key-logs-pane').hide();
            $('#projects').show();
        } else if( nav_target === 'tools' ) {
            $('#projects, #refs-pane, #key-logs-pane').hide();
            $('#tools-pane').show();
        } else if ( nav_target === 'refs' ) {
            $('#projects, #tools-pane,  #key-logs-pane').hide();
            $('#refs-pane').show();
        } else if ( nav_target === 'key_logs' ) {
            $('#projects, #tools-pane,  #refs-pane').hide();
            $('#key-logs-pane').show();
        }
    });
    ...
    $('input[name="key_logging').change(function(e){
        if( $(this).is(':checked') ) {
            ...
            ipcRenderer.send('key_logger', true);
        } else {
            $('.key-logger-status').css('color', 'red');
            ipcRenderer.send('key_logger', false);
        }
    });
