...
ipcRenderer.on('key_log', (e, key) => {
    ...
    <li class="ui-state-default key-log" data="${key}">
        <div class="row clickable">
            ...
            <div class="col-2"><i class="fa fa-close remove-key-log clickable"></i></div>
        </div>
    </li>`;
...
$('document').ready(function(e) {
    ...
    $('.sidebar-nav').click(function(e) {
        ...
        if(  nav_target === 'projects' ) {
            $('#tools-pane, #refs-pane').hide();
            $('#projects').show();
        } else if( nav_target === 'tools' ) {
            $('#projects, #refs-pane').hide();
            $('#tools-pane').show();
        } else if ( nav_target === 'refs' ) {
            $('#projects, #tools-pane').hide();
            $('#refs-pane').show();
        }
    });
    ...
    $('.logger-menu-item').click(function(e) {
        ...
        let state = 'stopped';
        // let roku = JSON.parse($('#roku-select option:selected').attr('data'));
        let roku = {ip_address: '10.10.15.50'};
