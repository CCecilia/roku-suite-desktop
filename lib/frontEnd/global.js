    ...
    let roku_html = `
        <div class="col-3 roku-card clickable" draggable="true" ondragstart="cardDrag(event, 'roku')" data-id="${roku._id}">
            <div class="row">
                ...
                <div class="col-12 text-center">
                    <button class="btn btn-block btn-roku restart-roku" data='${roku_data}'>Restart</button>
                </div>
            </div>
        </div>
    `;
        ...
        <div class="col-3 project-card clickable" draggable="true" ondragstart="cardDrag(event, 'project')" ondragend="toggleTrash()" data-id="${project._id}">
            <div class="row">
                ...
                <div class="col-12 text-center pad-4">
                    ...
                    <button class="btn btn-roku btn-block lint-btn" data-id="${project._id}">
                        Lint
...
function restartRoku(roku) {
    // spoofing element in log 
    let restart_sequence = [
        {key: 'home', element: $('spoof-element')},
        {key: 'up', element: $('spoof-element')},
        {key: 'select', element: $('spoof-element')},
        {key: 'up', element: $('spoof-element')},
        {key: 'select', element: $('spoof-element')},
        {key: 'up', element: $('spoof-element')},
        {key: 'up', element: $('spoof-element')},
    ...
    $.notify({
        message: 'Roku restarting'
    },{
        type: 'success'
    });

    // recursively iterate thgrough restart sequence
    function runRestartSequence() {
        let log = restart_sequence.shift();
        
        sendKeyPressAsync(roku, log)
            .then(() => {
                if( restart_sequence.length != 0) {
                    setTimeout(function() {
                        runRestartSequence();
                    }, 1000)
                }
...
// handle mouse locking/select keypress
if( canvas ) {
    canvas.onclick = function() {
        if( mouse_locked ) {
            ...
            for( let i=0; i<rokus.length; i++ ) {
                sendKeyPressSync(JSON.parse($(rokus[i]).attr('data')), 'select')
            }
        } else {
...
// handle mouse unlocking from canvas
...
$(document).keypress(function(e) {
    if( mouse_locked ) {
        ...
        if( key === 'Q' ) {
            ...
            roku_key = `Lit_${encodeURIComponent(key)}`;
        }

        if( roku_key ) {
            for( let i=0; i<rokus.length; i++ ) {
                sendKeyPressSync(JSON.parse($(rokus[i]).attr('data')), roku_key);
            }
        }
    }
});
...
function handleMouseMovement(new_point) {
    // +/- 30 to x,y to set a higher threshold to lower movement detection
    let movement;
    if( new_point.y > last_point.y + 30 ) {
        movement = 'down';
        last_point = new_point;
    } else if ( new_point.y < last_point.y - 30 ) {
        movement = 'up';
        last_point = new_point;
    } else if ( new_point.x > last_point.x + 30 ) {
        movement = 'right';
        last_point = new_point;
    } else if ( new_point.x < last_point.x - 30 ) {
        movement = 'left';
    ...
    if( movement ) {
        ...
        for( let i=0; i<rokus.length; i++ ) {
            sendKeyPressSync(JSON.parse($(rokus[i]).attr('data')), movement)
        }
    }
}

function updatePosition(e) {
    // update circle's position on canvas 
    x += e.movementX;
    y += e.movementY;
...
ipcRenderer.on('updateAnalytics', (evt, payload) => {
    reportedEvents.push(payload);
    const container = $("<div class='analytics-event'></div>");
    const eventId = payload.properties.event_id;
    container.attr('id', eventId);
    $('#analytics-data-view').append(container);
    jsonView.format(payload.event, JSON.stringify(payload.properties), '#' + eventId);
});
...
// auto deploy signaled from project watchers
ipcRenderer.on('restart_prompt', (e, roku) => {
    console.log('restart_prompt');
    $.notify({
    },{
        timer: 5000,
        template: 
        '<div data-notify="container" class="col-xs-11 col-sm-3 alert alert-{0}" role="alert" style="background-color:#662d91;">' +
            '<button type="button" aria-hidden="true" class="close" data-notify="dismiss">×</button>' +
            '<span data-notify="icon"><button class="btn btn-roku-outline restart-roku" data=\''+JSON.stringify(roku)+'\'>Restart Roku</button></span> ' +
        '</div>' 
    });
});
...
$('document').ready(function(e) {
    ...
    $('.sidebar-nav').click(function(e) {
        ...
        const allViews = '#projects, #tools-pane, #refs-pane, #key-logs-pane, #rokus-pane, #analytics-pane'
        ipcRenderer.send('analytics_ignore');
        ...
        if(  nav_target === 'projects' ) {
            ...
            $(allViews).hide();
            $('#tools-pane').show();
        }else if( nav_target === 'analytics' ) {
            ipcRenderer.send('analytics_listen');
            $(allViews).hide();
            $('#analytics-pane').show();
    ...
    // handle roku restart
    $('.restart-roku').click(function(e) {
        let roku = JSON.parse($(this).attr('data'));
