 ...
 * @return {HTMLElement}
 */
function createExpandedElement(label, node) {
 ...
 */
function traverseObject(label, obj, parent) {
  for (let key in obj) {
    ...
    if (typeof obj[key] === 'object') {
      ...
      traverseObject(label, obj[key], child);
      child.elem = createExpandedElement(label, child);
    } else {
      child.value = obj[key];
...
function createTree(label, obj) {
  ...
  traverseObject(label, obj, tree);
  tree.elem = createExpandedElement(label, tree);
...
window.jsonView = {
   ...
   * @param {String} targetElem
   */
  format: function(label, jsonData, targetElem) {
    const parsedData = JSON.parse(jsonData);
    const tree = createTree(label, parsedData);
