    ...
    autoDeploy: (project_id, mainWindow) => {
        async.parallel({
            ...
            function compileDirsToBeWatched(project) {
                console.log('compileto_be_watched');
                return new Promise((resolve, reject) => {
            ...
            function killWatchers() {
                return new Promise((resolve, reject) => {
                    if( active_watchers.length > 0 ) {
                        for( let i=0; i<active_watchers.length; i++) {
                            if( !active_watchers[i].isClosed() ){
                                active_watchers[i].close();
                            }

                            if( i === active_watchers.length ) {
                                resolve(true)
                            }
                        }
                    } else {
    ...
    checkGitBranches: (projects) => {
        for( let i=0; i<projects.length; i++ ){
            ...
            branch(projects[i].root_path)
                .then((name) => {
                    //  check branch
                    console.log(typeof(name), typeof(projects[i].git_branch))
                    console.log(name, projects[i].git_branch)
